...
/** Checks if the appropriate output is a servo and if yes initiates the control pulse. 
 
    The pulse is between 0.5 and 2.5ms. 
 */
void servoTick() {
    ...
    if (outputs_[i].kind == OutputKind::Servo) {
        ...
        //DDISP(0,0, "Servo    ");
        //TCB0.CCMP = 1650 + (outputs_[i].value & 0xff) * 26;
