...
public:
    ...
    Keyboard(Window * window): Widget{window} {}

protected:
    void draw() override {
        DrawTextEx(window()->helpFont(), prompt_.c_str(), 0, 20, 16, 1.0, GRAY);
    ...
    void drawButton(int x, int y, char const * text) {
        ...
        Vector2 size = MeasureText(window()->menuFont(), text, 28);
        DrawTextEx(window()->menuFont(), text, x + (28 - size.x) / 2, y, 28, 1.0, WHITE);
    }
